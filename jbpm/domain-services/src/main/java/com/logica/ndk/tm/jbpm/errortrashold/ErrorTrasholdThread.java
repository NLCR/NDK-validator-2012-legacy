 ...
 * @author brizat
 */
public class ErrorTrasholdThread implements Runnable {

  public static int ERROR_STATE_CODE = 3;
  protected static Logger LOG = LoggerFactory.getLogger(ErrorTrasholdThread.class);
  ...
  public synchronized void run() {
    ...
    LOG.info("Check started! Error process " + processId);
    try {
      boolean change = false;
      ProcessRuntimeConfig loadConfig = ConfigLoader.loadConfig();
      //find process config node
      for (ProcessConfig processConfig : loadConfig.getProcess()) {
        if (processConfig.getProcessId().equals(processId)) {
