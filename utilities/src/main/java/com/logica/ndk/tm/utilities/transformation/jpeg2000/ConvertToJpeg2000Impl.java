...
public class ConvertToJpeg2000Impl extends AbstractUtility {
  ...
  private String COMMON_PATH = "utility.convertToJpeg2000.profile.";
  ...
  public Integer execute(String cdmId, String source, String target, String profile, String sourceExt) throws TransformationException {
    ...
    if ((profile == null || profile.isEmpty()) && migrationHelper.isFormatMigration(cdm.getCdmProperties(cdmId).getProperty("importType"))) {
      profileFullName = COMMON_PATH + migrationHelper.getProfile(new File(target));
    }
    else {
      profileFullName = COMMON_PATH + profile;
    }
    ...
    for (final File file : listFiles) {
      if (!file.isDirectory()) {
        log.info("File to transform: " + file.getAbsolutePath());
  ...
  private void generateEvent(final KakaduService agent, final File file, final String cdmId, final PremisCsvRecord.OperationStatus status, final File targetDir) {
    final PremisCsvRecord record = new PremisCsvRecord(
        ...
        getUtlilityName(),
        getUtilityVersion(),
        Operation.migration_mc_creation,
        targetDir.getName(),
        agentName,
        agentVersion,
        agent.getCmd(),
