...
class ArchiveRecord(object):
    ...
    def dump_to_file(self, directory, onlyResponse=True):
        fileName = self.get_header("WARC-Record-ID").replace("<", "").replace(">", "").replace(":", "-")
        pathMain = directory + os.sep + fileName + ".data"
        pathMeta = directory + os.sep + fileName + ".meta"
        content_type, content_body = self.content
        if onlyResponse:
            if "msgtype=response" in content_type: 
                # meta file
                FILE = open(pathMeta, "w")
                for (h, v) in self.headers:
                    FILE.write(h)
                    FILE.write(":")
                    FILE.write(v)
